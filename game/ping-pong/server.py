...
class GameServer:
    ...
    def bot_logic(self):
        while not self.game_over:
            with self.lock:
                target_y = self.ball['y'] + random.randint(-20, 20)
                paddle_center = self.paddles[1] + 50

                if paddle_center < target_y:
                    self.paddles[1] = min(HEIGHT - 100, self.paddles[1] + PADDLE_SPEED)
                elif paddle_center > target_y:
                    self.paddles[1] = max(60, self.paddles[1] - PADDLE_SPEED)
    ...
    def run(self):
        while True:
            ...
            while not self.game_over and self.connected[0]:
                time.sleep(0.1)
